...
- Two-stage handshake is mandatory before file mutation:
  1. Call \`select_active_intent\` with a valid \`intent_id\`.
  2. Include matching \`intent_id\` metadata on write operations (for example \`write_to_file\`).
- If no valid active intent has been selected, do not execute write operations.
